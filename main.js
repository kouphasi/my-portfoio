...
const tennisButton = doccument.getElementById("tennis");
const soccerButton = doccument.getElementById("soccer");
...
const checkAcuracy = (name) => {
  ...
  for (let i = 0; i < name.length; i++) {
    let x = indexOf(name[i]);

    if (x != -1) {
      distance_list.push(Math.abs(x - r));
    } else {
